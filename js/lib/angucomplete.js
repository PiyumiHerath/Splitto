    ...
    return {
        ...
        link: function($scope, elem, attrs) {
            ...
            $scope.selectResult = function(result) {
                ...
                $scope.selectedObjects.push( result.originalObject.name );
                console.log(result);
                for( var i = 0; i < $scope.localData.length; i++ ) {
                  if ( $scope.localData[i] === result.originalObject ) {
                    $scope.localData.splice(i, 1);
                  }
                }
                //$scope.localData.splice($scope.index, 1);
                //$scope.selectedObject.push(result.originalObject);
                $scope.showDropdown = false;
                $scope.results = [];
                //$scope.$apply();
            }
            ...
            elem.on("keyup", function (event) {
                if(event.which === 40) {
                    ...
                    if ($scope.results && $scope.currentIndex >= 0 && $scope.currentIndex < $scope.results.length) {
                        ...
                        $scope.$apply();
                        event.preventDefault;
                        //$scope.localData.splice($scope.currentIndex,1);
                        event.stopPropagation();
                        $('#expenseUsers_value').val('');
                    } else {
                        $scope.results = [];
                        $scope.$apply();
                        event.preventDefault;
